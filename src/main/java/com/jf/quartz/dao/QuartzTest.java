...
public class QuartzTest {
    ...
    public int addCronJob(String jobName, Class<? extends Job> job, Object task, int seconds, String jobGorupName) {
        try {
            // 判断任务是否存在
            JobKey jobKey = JobKey.jobKey(jobName, jobGorupName);
            if (scheduler.checkExists(jobKey)) {
            ...
            cronTrigger.setStartTime(new Date(new Date().getTime() + 1000 * seconds));
            cronTrigger.setCronExpression("0/5 * * * * ? ");
            cronTrigger.setTimeZone(TimeZone.getDefault());
            cronTrigger.setJobGroup(jobGorupName);
